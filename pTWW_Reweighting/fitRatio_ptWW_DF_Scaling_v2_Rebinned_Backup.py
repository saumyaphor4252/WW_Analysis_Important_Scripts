...
#    histoMC_Bg.Add(histo2016noHIPMggWW)
    ...
    histoMC_Bg_rebinned.GetXaxis().SetTitle("pTWW [GeV]")
    histoMC_Bg_rebinned.GetYaxis().SetTitle("Fraction of events")
    ...
    # Only fit and plot pol6
    order = 6
    tf = ROOT.TF1("tf6", "pol6", 0, 200)
    fit_result = histo2018DATA_rebinned.Fit("tf6", "S", "", 0, 200)
    chi2 = fit_result.Chi2()
    ndf = fit_result.Ndf()
    chi2_ndf = chi2/ndf if ndf > 0 else float('inf')
    print("Polynomial order 6: Chi2/NDF = %.3f" % chi2_ndf)

    # Draw histogram
    histo2018DATA_rebinned.Draw()
    tf.SetLineColor(ROOT.kRed)
    tf.Draw("same")

    # Create legend
    legend = ROOT.TLegend(0.7, 0.7, 0.9, 0.9)
    legend.SetHeader("Polynomial Order")
    legend.AddEntry(tf, "pol6 (chi2/NDF=%.2f)" % chi2_ndf, "l")
    legend.Draw()
